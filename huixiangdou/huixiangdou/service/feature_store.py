...
def parse_args():
    ...
    parser.add_argument(
        '--repo_dir',
        type=str,
        default='/group_share/data/knowleage',
...
def test_reject(retriever: Retriever, sample: str = None,args =None):
    ...
    for example in real_questions:
        ...
        if sample is not None:
            if reject:
                with open(args.work_dir+'/negative.txt', 'a+') as f:
                    f.write(example)
                    f.write('\n')
            else:
                with open(args.work_dir+'/positive.txt', 'a+') as f:
                    f.write(example)
                    f.write('\n')
...
if __name__ == '__main__':
    ...
    # test
    retriever = cache.get(config_path=args.config_path, work_dir=args.work_dir)
    test_reject(retriever, args.sample,args=args)
